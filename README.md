...
`blog.js` 分 7 个模块，全部包裹在外层 IIFE 中防止全局污染，加载后自行按容器是否存在初始化（三个页面共用同一份脚本，无需各自的 init 代码）：

| 模块             | 功能                                                             |
| ---------------- | ---------------------------------------------------------------- |
| `renderMarkdown` | 全站唯一渲染点（禁原始 HTML、协议白名单、图片 lazy）             |
| `BlogList`       | 列表页：fetch `posts.json` 元数据渲染卡片                        |
| `BlogSearch`     | 列表页搜索：元数据 + 正文全文（见下），150ms 防抖，`?q=` 可恢复  |
...
匹配范围是标题 / 摘要 / 标签 / slug **加上正文全文**。正文索引在**首次输入时**才懒加载——按 `posts.json` 的 slug 列表逐个 fetch `blog/md/<slug>.md`（当前 5 篇共约 48 KB），剥掉 frontmatter、正则粗剥 markdown 语法后留在内存，不搜的人不会多付这几个请求。

因为列表页没有引入 `marked.min.js`，建索引用的是手写的 `toSearchText()` 而不是 marked（为建索引加载一个 40 KB 的渲染器比正文本身还重）。它**刻意保留代码块内容**，这样搜命令、搜报错信息都能命中。

命中只出现在正文里时，卡片下方会多一行**引用样式的命中片段**（沿用文章页 `blockquote` 的视觉），避免出现"看不出为什么匹配"的结果。URL 会随搜索同步 `?q=`，可直接分享。

> 新增文章**不需要**改任何索引文件——搜索索引是运行时按 `posts.json` 现拉的，与构建流程完全无关。
