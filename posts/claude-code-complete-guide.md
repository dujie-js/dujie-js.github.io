...
国内没法直接访问 Anthropic API，Claude Code 装好了用不了。需要走第三方中转或者用国产模型（DeepSeek、智谱这些）来替代。每次手动改 `~/.claude/settings.json` 切配置太烦了，然后发现了 **cc-switch**。
...
**Windows：** 去 [GitHub Releases](https://github.com/farion1231/cc-switch/releases) 下载 `.msi` 安装包，双击装就行。
...
如果用第三方 API，Claude Code 第一次启动会弹登录引导。在 cc-switch 设置里打开「跳过初次安装确认」就行了。
